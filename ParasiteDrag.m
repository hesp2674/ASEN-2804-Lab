...
v1_x_c= Airfoil.X_thick_v1;
v1_t_c= Airfoil.Thick_v1;
v1_sweep= Design_Input.Sweep_v1;
v1_Swet= Design_Input.Swet_v1;
v2_x_c= Airfoil.X_thick_v2;
v2_t_c= Airfoil.Thick_v2;
v2_sweep= Design_Input.Sweep_v2;
v2_Swet= Design_Input.Swet_v2;
Surface_finish=0.635*(10^(-5)); %Changes k value

Re_h = 5*(10^(5));
MAC_h1=Design_Input.MAC_h1;
MAC_h2=Design_Input.MAC_h2;
MAC_v1=Design_Input.MAC_v1;
MAC_v2=Design_Input.MAC_v2;

h1_x_c= Airfoil.X_thick_h1;
h1_t_c= Airfoil.Thick_h1;
h1_taper= Design_Input.Taper_h1;
h1_sweep = Design_Input.Sweep_h1;
...
X_thick_w = Airfoil.X_thick_w;
Thick_w = Airfoil.Thick_w;
Sweep_w = Design_Input.Sweep_w;
Q_w = Design_Input.Q_w;
Swet_w = Design_Input.Swet_w; 
...
% /////////////////////////////////////////////////////////////////////////
    ...
    Re_f_L = (Velocity(n)*Length_f(n))/Nu_air(n); %Re for fuselage %note possibly multiply air rho
    ...
    F_int = (Length_f(n))/(sqrt((4*A_max(n))/pi));
    FF_f(n) = ((.9)+(5/(F_int)^1.5)+(F_int)/400); %Fuselage Form Factor

    CDo_f(n) = Cf_f*FF_f(n)*Swet_f(n)*Q_f(n)/Sref_w(n); %Contribution of Fuselage to CDo

    %% Wing Contribution to CDo
    b_w = sqrt(AR_w(n)*Sref_w(n));
    Cr_w = (2*Sref_w(n))/(b_w(n)*(1+Taper_w(n)));
    MAC_w = (2*Cr_w(n)*(1+Taper_w(n)+(Taper_w(n))^2)/(3*(1+Taper_w(n))));
    Re_w = (Velocity(n)*MAC_w(n))/(Nu_air(n)); %Wing Re
    Cf_w = .074/(Re_w^0.2); %Wing Flat Plate Coef of Friction for Turbulent Flow
    FF_w(n) = (1 + ((.6/X_thick_w(n))*(Thick_w(n))) + (100*Thick_w(n)^4))*(1.35*(Mach(n)^.18)*(cosd(Sweep_w(n))^.28)); %Wing Form Factor
    if FF_w(n)<1
        FF_w(n)=1;
    end

    CDo_w(n) = (Cf_w*FF_w(n)*Q_w(n)*Swet_w(n))/(Sref_w(n)); %Contribution of Wing to CDo
    ...
    if Design_Input.Swet_h1(n)~=0 % If this component exists:
        Re_h1 = (Velocity(n)*MAC_h1(n))/Nu_air(n);
        Cf_h1 = 0.074/(Re_h1(n)^.2); %Flat Plate Coef of Friction for Turbulent Flow
        FF_h1(n) = (1+(.6/h1_x_c(n))*h1_t_c(n)+100*(h1_t_c(n))^4)*(1.35*(Mach(n)^.18))*(cos(h1_sweep(n)))^.28; %Horz Tail Form Factor
        if FF_h1(n)<1
            FF_h1(n)=1;
        end

        CDo_h1(n) = (Cf_h1(n)*FF_h1(n)*h1_Q(n)*h1_wet(n))/Sref_w(n); %Contribution of Horz Tail 1 to CDo
    end

    %% Horizontal Tail #2 Contribution to CDo
    if Design_Input.Swet_h2(n)~=0 % If this component exists:
        Re_h2 = (Velocity(n)*MAC_h2(n))/Nu_air(n);
        Cf_h2 = 0.074/(Re_h2^.2); %Flat Plate Coef of Friction for Turbulent Flow
        FF_h2(n) = (1+(.6/h2_x_c)*h2_t_c+100*(h2_t_c)^4)*(1.35*(Mach(n)^.18))*(cos(h2_sweep))^.28; %Horz Tail Form Factor
        if FF_h2(n)<1
            FF_h2(n)=1;
        end
        CDo_h2(n) = Cf_h2*FF_h2*h2_Q*h2_wet/Sref_w(n); %Contribution of Horz Tail 2 to CDo 
    end

    %% Vertical Tail #1 Contribution to CDo
    if Design_Input.Swet_v1(n)~=0 % If this component exists:
        Re_v1 = (Velocity(n)*MAC_v1(n))/Nu_air(n);
        Cf_v1 = 0.074/(Re_v1)^0.2; %Flat Plate Coef of Friction for Turbulent Flow
        FF_v1(n) = (1+(0.6/(v1_x_c(n)))*(v1_t_c(n))+100*(v1_t_c(n))^4)*(1.35*(Mach(n)^0.18)*cos(v1_sweep(n))^0.28); %Horz Tail Form Factor
        if FF_v1(n)<1
            FF_v1(n)=1;
        end

        CDo_v1(n) = Cf_v1*FF_v1(n)*v1_Q*v1_Swet(n)/Sref_w(n); %Contribution of Vert Tail 1 to CDo 
    end

    %% Vertical Tail #2 Contribution to CDo
    if Design_Input.Swet_v2(n)~=0 % If this component exists:
        Re_v2 = (Velocity(n)*MAC_v2(n))/Nu_air(n);
        Cf_v2 = 0.074/(Re_v2)^0.2; %Flat Plate Coef of Friction for Turbulent Flow
        FF_v2(n) = (1+(0.6/(v2_x_c(n)))*(v2_t_c(n))+100*(v2_t_c(n))^4)*(1.35*(Mach(n)^0.18)*cos(v2_sweep(n))^0.28); %Horz Tail Form Factor
        if FF_v2(n)<1
            FF_v2(n)=1;
        end
        CDo_v2(n) = Cf_v2*FF_v2(n)*v2_Q*v2_Swet(n)/Sref_w(n); %Contribution of Vert Tail 1 to CDo
    end

    %% Misc. and L&P Contributions to CDo
    if Design_Input.Abase_f(n)~=0 % If this component exists:
        D_q_base = (Mach(n)-0.161)^2;
        D_q_base = (.419*D_q_base) + .139;
        D_q_base = D_q_base*Abase_f(n);

        %NEED CLARIFICATION ON WHAT Sref IS
        CDo_misc(n) = D_q_base*(1/Sref_w(n)); %Contribution of base drag to CDo
    end
    ...
    Swet_tot(n) = h1_wet + h2_wet  + Swet_w(n) + Swet_f(n)+ v1_wet(n)+ v2_wet(n); %Total Wetted Area
