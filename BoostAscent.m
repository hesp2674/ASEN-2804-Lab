...
for n = 1:Count
    ...
    m_empty = (Weight_Data.(1) - Weight_Data.(9))/g; % [kg] empty weight of the vehicle (not including the water)   ****Might break due to table
    m0 = Weight_Data.(1)/g; % Note that the input water volume should be in ml which approx = grams
    Wx = Design_Input.V_wind(n) * cosd(Design_Input.Wind_Az(n)); % [m/s] Wind velocity in the x-direction (calculate using speed and heading, x = north)
    Wy = Design_Input.V_wind(n) * sind(Design_Input.Wind_Az(n)); % [m/s] Wind velocity in the y-direction (y = east)
    ...
    consts(8) = m_empty(n); % Weight of the rocet with no water [kg]
    % Wind
    ...
    S0(7) = m0(n); % current total mass [kg]
    ...
    [apogee(n), iApogee] = max(abs(S(:, 6)));  % abs inside max()
